...
generateCardsWithSize size symbols = let
  go :: State -> Set.Set Card -> Set.Set Card
  go state@(State size _ _) cards = if (cardIsSize card' size)
      then go newState (Set.insert card' cards)
    else cards
    where (newState, card') = makeCard state
...
makeCard (State size unusedSymbols onceUsedSymbols) = let
  unmatched = Set.take (size - 1) unusedSymbols
...
symbols = toSet ["hand", "eye", "brain", "tooth", "bone", "muscle", "baby", "ball", "violin", "watch", "phone", "laptop", "keyboard", "printer", "joystick", "cd", "floppydisk", "vhs", "camera"]
